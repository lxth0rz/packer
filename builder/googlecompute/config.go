...
type Config struct {
	...
	// setting the `enable-oslogin` to `TRUE` in the instance metadata.
	// OSLogin requires that a user's SSH key be added to their Google Account - See [Adding SSH Keys](https://cloud.google.com/compute/docs/instances/managing-instance-access#add_oslogin_keys).
	//
	...
	//```
	//
	// Or SSH keys can be added to a user's associated service account
	//```shell-session
